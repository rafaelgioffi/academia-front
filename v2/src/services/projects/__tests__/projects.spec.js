...
describe("[services] Projects", () => {
  describe("[GET VERB]", () => {
    ...
    it("fetches successfully data from a get promotions id", async () => {
      await expect(ProjectServices.getProjectById(idProject)).resolves.toEqual(
        data
      );
  ...
  describe("[POST VERB]", () => {
    it("fetches successfully data from a get a new promotion with data", async () => {
      await expect(
        ProjectServices.saveProjects(MOCK_WITH_POST_AND_PUT_PROJECTS)
      ).resolves.toEqual(data);

      expect(mockFn).toHaveBeenCalledWith(
        projectsBasePath,
        MOCK_WITH_POST_AND_PUT_PROJECTS
      );
    });
  });
  ...
  describe("[DELETE VERB]", () => {
    it("fetches successfully data from a get a new promotion with data", async () => {
      await expect(ProjectServices.deleteProject(idProject)).resolves.toEqual(
        data
      );
